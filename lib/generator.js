...
function formatDate (d) {
  ...
  return d.getUTCFullYear().toString()
    + pad(d.getUTCMonth() + 1).toString()
    + pad(d.getUTCDate()).toString() + '-'
    + pad(d.getUTCHours()).toString()
    + pad(d.getUTCMinutes()).toString();
}
...
var generator = {
  ...
  register: function register (section, callback) {
    if (Object.keys(this._sections).indexOf(section) === -1) { throw new Error('unknown generate section: ' + section); }
