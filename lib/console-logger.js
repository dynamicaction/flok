...
function prettyDir (obj) {
  console.log(util.inspect(obj, false, 10, false).grey);
}
...
function dateString () {
  var d = new Date();
  return d.getDate() + ' ' + MONTHS[d.getMonth()] + ' ' + d.toLocaleTimeString() + ' - ';
}

function getLogger (level, desiredLevel, printObj, toStderr) {
  ...
  return function logger () {
    ...
    console[toStderr ? 'error' : 'log'](dateString() + dl(level) + m);
...
module.exports = function ConsoleLogger (o) {
  ...
  this.log = {
    ...
    error: getLogger('error', level, objects, true),
    alert: getLogger('alert', level, objects, true),
    fatal: getLogger('fatal', level, objects, true)
  };
};
