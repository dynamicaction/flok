...
module.exports.saveMigrationStatus = function saveMigrationStatus (status, migration, done) {
  ...
  migration.log.debug({path: p, status: status}, 'writing migration status file to %s', p);
...
module.exports.clearMigrationStatus = function clearMigrationStatus (migration, done) {
  var p = this.migrationStatusDir || path.join(this.migrationsDir, 'flokStatus');
  p = path.join(p, slugify(migration.id)) + '.json';

  if (!fs.existsSync(p)) {
    migration.log.warn(migration, 'migration does not have a status file to remove: %s', p);
    return process.nextTick(done.bind(null, null));
  }
