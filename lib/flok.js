...
Flok.prototype._run = function _run (context, method, args, done) {
  ...
  if (!method.name) { this.log.warn({stack: (new Error()).stack}, 'Flok._run called with unnamed method'); }
...
Flok.prototype.up = function up (opts, done) {
  ...
  async.auto({
    ...
    loadMigrations: ['lock', function (callback) {
      self.load(callback);
    }],

    incompatibleMigrations: ['loadMigrations', (callback) => {
      if (self.firstRun) return callback();
      this.checkV1Migrations(callback);
    }],

    changedMigrations: ['incompatibleMigrations', function (callback) {
...
Flok.prototype.down = function down (opts, done) {
  ...
  async.auto({
    ...
    sortMigrations: ['loadMigrations', function (callback) {
    ...
    executeMigrations: ['blockedMigrations', function (callback) {
      var migrations = filter(self.migrations, 'isDone').slice(0, 1);
      if (migrations[0] && migrations[0].isIncompatible) {
        self.log.error(`migration ${migrations[0].id} is incompatible with this version of flok`);
        return (callback());
      }
      self._executeMigrations(migrations, '_down', callback);
    }],
...
Flok.prototype.load = function load (done) {
  ...
  async.auto({
    ...
    validateMigrations: ['migrations', function (callback) {
      self._validateMigrations(callback);
    }],
    migrationStatus: ['migrations', function (callback) {
      self._loadMigrationsStatus(callback);
    }],
    sortMigrations: ['validateMigrations', 'migrationStatus', function (callback) {
...
Flok.prototype._validateMigrations = function _validateMigrations (callback) {
  var ids = pluck(this.migrations, 'id');
  var invalid = 0;
  var log = this.log;

  this.migrations.forEach(function (mig) {
    if (!mig.dependencies) return;
    mig.dependencies.forEach(function (dep) {
      if (ids.indexOf(dep) >= 0) return;

      invalid++;
      log.error('[' + dep + '] is not a valid dependency for ' + mig.id);
    });
  });

  if (invalid >= 1) return callback(new Error('found ' + invalid + ' invalid migrations'));
  callback();
};
...
Flok.prototype.checkV1Migrations = function checkV1Migrations (callback) {
  var v1 = this.migrations.filter((m) => m.isPending && m.isIncompatible);
  this.log.debug(`found ${v1.length} pending migrations incompatible with this version`);
  if (v1.length > 0) {
    this.log.warn(`${v1.length} pending migrations are not compatible with this version of flok`);
    return callback(new Error('found incompatible migrations'), v1);
  }
  callback(null, []);
};
  ...
  async.auto({
    ...
    verifyMigrationsDir: function (callback) {
      if (!fs.existsSync(migrationsDir)) {
        return callback(new Error('no directory found at ' + migrationsDir));
      }
      return callback();
    },
...
const SKIP_MIGRATION = Symbol('SKIP_MIGRATION');
  ...
  async.whilst(
    ...
    function doExecute (doneOne) {
      async.auto({
        migration: function (cb) {
          self._run(self, self.dequeue, [pending], function (err, m) {
            ...
            if (m.flokVersion.startsWith('1')) {
              m.skip();
              return cb(SKIP_MIGRATION, m);
            }
      ...
      function (err, res) {
        ...
        if (err) {
          if (err === SKIP_MIGRATION) {
            self.log.info(`skipped execution of v1 migration: [${m.id}] ${m.title}`);
            err = null;
          } else {
            errored = true;
